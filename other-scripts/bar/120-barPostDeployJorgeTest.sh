...
function mongoRestore()
{
  ...
  # This required because these are registered in the MongoDB that has
  ...
  logWarning "You must restore Zen data and services to properly access the CloudPak "
}
...
function restoreZen()
{
  ...
  echo $newClientID $newClientSecret $oldClientID $oldClientSecret
  ...
  do
    ...
    oc exec $pod -it -- bash -c "cd /user-home && sed -i -f switch.sed zen-metastoredb-backup/oidc/oidcConfig.json && cat zen-metastoredb-backup/oidc/oidcConfig.json"
