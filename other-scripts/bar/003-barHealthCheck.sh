...
logInfo "Saving CP4BA CR to ${CUR_DIR}/CR.json..."
oc get ICP4ACluster $CP4BA_NAME -o json > ${CUR_DIR}/CR.json

##### CP4BA Overall status #####################################################
# Check CP4BA deploymnet status
logInfo "Checking CP4BA Deployment overall status..."

CP4BA_DEPLOYMENT_STATUS=$(jq -r .status.conditions ${CUR_DIR}/CR.json | jq -r '.[] |select(.type == "Ready") | .status')
checkResult $CP4BA_DEPLOYMENT_STATUS "True" "CP4BA deployment status"

CP4BA_RECONCILIATION_STATUS=$(jq -r .status.conditions ${CUR_DIR}/CR.json | jq -r '.[] |select(.type == "Running") | .status')
checkResult $CP4BA_RECONCILIATION_STATUS "True" "CP4BA reconciliation running"

CP4BA_PREREQ_STATUS=$(jq -r .status.conditions ${CUR_DIR}/CR.json | jq -r '.[] |select(.type == "PrereqReady") | .status')
...
CP4BA_COMPONENTS=$(jq -r .spec.shared_configuration.sc_deployment_patterns ${CUR_DIR}/CR.json)
logInfo "  CP4BA deployment patterns: $CP4BA_COMPONENTS"

CP4BA_OPTIONAL_COMPONENTS=$(jq -r .spec.shared_configuration.sc_optional_components ${CUR_DIR}/CR.json)
...
CP4BA_PREREQ_IAF=$(jq -r .status.components.prereq.iafStatus ${CUR_DIR}/CR.json)
checkResult $CP4BA_PREREQ_IAF "Ready" "CP4BA Prereq iafStatus"

CP4BA_PREREQ_IAM=$(jq -r .status.components.prereq.iamIntegrationStatus ${CUR_DIR}/CR.json)
checkResult $CP4BA_PREREQ_IAM "Ready" "CP4BA Prereq iamIntegrationStatus"

##### Foundation pattern #######################################################
if [[ $CP4BA_COMPONENTS =~ "foundation" ]]; then
  # RR
  ...
  CP4BA_NAVIGATOR_DEPLOYMENT=$(jq -r .status.components.navigator.navigatorDeployment ${CUR_DIR}/CR.json)
  checkResult $CP4BA_NAVIGATOR_DEPLOYMENT "Ready" "CP4BA Navigator Deployment"
  
  CP4BA_NAVIGATOR_SERVICE=$(jq -r .status.components.navigator.navigatorService ${CUR_DIR}/CR.json)
  checkResult $CP4BA_NAVIGATOR_SERVICE "Ready" "CP4BA Navigator Service"

  CP4BA_NAVIGATOR_STORAGE=$(jq -r .status.components.navigator.navigatorStorage ${CUR_DIR}/CR.json)
  checkResult $CP4BA_NAVIGATOR_STORAGE "Ready" "CP4BA Navigator Storage"

  CP4BA_NAVIGATOR_ZENINTEGRATION=$(jq -r .status.components.navigator.navigatorZenIntegration ${CUR_DIR}/CR.json)
  ...
  # BAS
  if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "bas" ]]; then
    logInfo "Checking Business Automation Studio..."
    if [[ ! $CP4BA_VERSION =~ "21.0.3" ]]; then
      # 21.0.3 doesn't have this property
      CP4BA_BAS_CAPABILITIES=$(jq -r .status.components.bastudio.capabilities ${CUR_DIR}/CR.json)
      logInfo "  Business Automation Studio capabilities enabled: $CP4BA_BAS_CAPABILITIES"
    fi
    CP4BA_BAS_STATUS=$(jq -r .status.components.bastudio.service ${CUR_DIR}/CR.json)
  ...
  if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "bas" ]]; then
    ...
    CP4BA_BAI_STATUS=$(jq -r .status.components.bai.bai_deploy_status ${CUR_DIR}/CR.json)
    checkResult $CP4BA_BAI_STATUS "Ready" "CP4BA Business Automation Insights deployment status"

    CP4BA_BAI_INSIGHTS_ENGINE_STATUS=$(jq -r .status.components.bai.insightsEngine ${CUR_DIR}/CR.json)
    checkResult $CP4BA_BAI_INSIGHTS_ENGINE_STATUS "Ready" "CP4BA Business Automation Insights Engine status"
  fi
fi

##### Content pattern ##########################################################
if [[ $CP4BA_COMPONENTS =~ "content" ]]; then
  # CPE
  logInfo "Checking Content Platform Engine..."
  CP4BA_CPE_DEPLOYMENT=$(jq -r .status.components.cpe.cpeDeployment ${CUR_DIR}/CR.json)
  checkResult $CP4BA_CPE_DEPLOYMENT "Ready" "CP4BA Content Platform Engine Deployment"
  
  CP4BA_CPE_SERVICE=$(jq -r .status.components.cpe.cpeService ${CUR_DIR}/CR.json)
  checkResult $CP4BA_CPE_SERVICE "Ready" "CP4BA Content Platform Engine Service"

  CP4BA_CPE_STORAGE=$(jq -r .status.components.cpe.cpeStorage ${CUR_DIR}/CR.json)
  checkResult $CP4BA_CPE_STORAGE "Ready" "CP4BA Content Platform Engine Storage"

  CP4BA_CPE_ZENINTEGRATION=$(jq -r .status.components.cpe.cpeZenIntegration ${CUR_DIR}/CR.json)
  checkResult $CP4BA_CPE_ZENINTEGRATION "Ready" "CP4BA Content Platform Engine Zen Integration"

  # GraphQL
  logInfo "Checking GraphQL..."
  CP4BA_GRAPHQL_DEPLOYMENT=$(jq -r .status.components.graphql.graphqlDeployment ${CUR_DIR}/CR.json)
  checkResult $CP4BA_GRAPHQL_DEPLOYMENT "Ready" "CP4BA GraphQL Deployment"
  
  CP4BA_GRAPHQL_SERVICE=$(jq -r .status.components.graphql.graphqlService ${CUR_DIR}/CR.json)
  checkResult $CP4BA_GRAPHQL_SERVICE "Ready" "CP4BA GraphQL Service"

  CP4BA_GRAPHQL_STORAGE=$(jq -r .status.components.graphql.graphqlStorage ${CUR_DIR}/CR.json)
  ...
  if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "css" ]]; then
    ...
    CP4BA_CSS_DEPLOYMENT=$(jq -r .status.components.css.cssDeployment ${CUR_DIR}/CR.json)
    checkResult $CP4BA_CSS_DEPLOYMENT "Ready" "CP4BA Content Search Service Deployment"

    CP4BA_CSS_SERVICE=$(jq -r .status.components.css.cssService ${CUR_DIR}/CR.json)
    checkResult $CP4BA_CSS_SERVICE "Ready" "CP4BA Content Search Service Service"

    CP4BA_CSS_STORAGE=$(jq -r .status.components.css.cssStorage ${CUR_DIR}/CR.json)
  ...
  if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "cmis" ]]; then
    ...
    CP4BA_CMIS_DEPLOYMENT=$(jq -r .status.components.cmis.cmisDeployment ${CUR_DIR}/CR.json)
    checkResult $CP4BA_CMIS_DEPLOYMENT "Ready" "CP4BA CMIS Deployment"

    CP4BA_CMIS_SERVICE=$(jq -r .status.components.cmis.cmisService ${CUR_DIR}/CR.json)
    checkResult $CP4BA_CMIS_SERVICE "Ready" "CP4BA CMIS Service"

    CP4BA_CMIS_STORAGE=$(jq -r .status.components.cmis.cmisStorage ${CUR_DIR}/CR.json)
    checkResult $CP4BA_CMIS_STORAGE "Ready" "CP4BA CMIS Storage"

    CP4BA_CMIS_ZENINTEGRATION=$(jq -r .status.components.cmis.cmisZenIntegration ${CUR_DIR}/CR.json)
  ...
  if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "tm" ]]; then
    ...
    CP4BA_TM_DEPLOYMENT=$(jq -r .status.components.tm.tmDeployment ${CUR_DIR}/CR.json)
    checkResult $CP4BA_TM_DEPLOYMENT "Ready" "CP4BA Task Manager Deployment"

    CP4BA_TM_SERVICE=$(jq -r .status.components.tm.tmService ${CUR_DIR}/CR.json)
    checkResult $CP4BA_TM_SERVICE "Ready" "CP4BA Task Manager Service"

    CP4BA_TM_STORAGE=$(jq -r .status.components.tm.tmStorage ${CUR_DIR}/CR.json)
  ...
  if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "ier" ]]; then
    ...
    CP4BA_IER_DEPLOYMENT=$(jq -r .status.components.ier.ierDeployment ${CUR_DIR}/CR.json)
    checkResult $CP4BA_IER_DEPLOYMENT "Ready" "CP4BA IER Deployment"

    CP4BA_IER_SERVICE=$(jq -r .status.components.ier.ierService ${CUR_DIR}/CR.json)
    checkResult $CP4BA_IER_SERVICE "Ready" "CP4BA IER Service"

    CP4BA_IER_STORAGE=$(jq -r .status.components.ier.ierStorageCheck ${CUR_DIR}/CR.json)
  ...
  if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "iccsap" ]]; then
    ...
    CP4BA_ICCSAP_DEPLOYMENT=$(jq -r .status.components.iccsap.iccsapDeployment ${CUR_DIR}/CR.json)
    checkResult $CP4BA_ICCSAP_DEPLOYMENT "Ready" "CP4BA ICCSAP Deployment"

    CP4BA_ICCSAP_SERVICE=$(jq -r .status.components.iccsap.iccsapService ${CUR_DIR}/CR.json)
    checkResult $CP4BA_ICCSAP_SERVICE "Ready" "CP4BA ICCSAP Service"

    CP4BA_ICCSAP_STORAGE=$(jq -r .status.components.iccsap.iccsapStorageCheck ${CUR_DIR}/CR.json)
  ...
  if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "extshare" ]]; then
    ...
    CP4BA_EXTSHARE_DEPLOYMENT=$(jq -r .status.components.extshare.extshareDeployment ${CUR_DIR}/CR.json)
    checkResult $CP4BA_EXTSHARE_DEPLOYMENT "Ready" "CP4BA External Share Deployment"

    CP4BA_EXTSHARE_SERVICE=$(jq -r .status.components.extshare.extshareService ${CUR_DIR}/CR.json)
    checkResult $CP4BA_EXTSHARE_SERVICE "Ready" "CP4BA External Share Service"

    CP4BA_EXTSHARE_STORAGE=$(jq -r .status.components.extshare.extshareStorage ${CUR_DIR}/CR.json)
...
if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "ads_runtime" ]]; then
  ...
  CP4BA_ADS_RUNTIME_SERVICE_DEPLOYMENT=$(jq -r .status.components.adsRuntimeService.adsRuntimeServiceDeployment ${CUR_DIR}/CR.json)
  checkResult $CP4BA_ADS_RUNTIME_SERVICE_DEPLOYMENT "Ready" "CP4BA Automation Decision Service deployment status"

  CP4BA_ADS_RUNTIME_SERVICE_SERVICE=$(jq -r .status.components.adsRuntimeService.adsRuntimeServiceService ${CUR_DIR}/CR.json)
  checkResult $CP4BA_ADS_RUNTIME_SERVICE_SERVICE "Ready" "CP4BA Automation Decision Service runtime service status"

  CP4BA_ADS_RUNTIME_SERVICE_ZENINTEGRATION=$(jq -r .status.components.adsRuntimeService.adsRuntimeServiceZenIntegration ${CUR_DIR}/CR.json)
  ...
  CP4BA_ADS_REST_APU_DEPLOYMENT=$(jq -r .status.components.adsRestApi.adsRestApiDeployment ${CUR_DIR}/CR.json)
  checkResult $CP4BA_ADS_REST_APU_DEPLOYMENT "Ready" "CP4BA Automation Decision Service Rest-API deployment status"

  CP4BA_ADS_REST_API_SERVICE=$(jq -r .status.components.adsRestApi.adsRestApiService ${CUR_DIR}/CR.json)
  checkResult $CP4BA_ADS_REST_API_SERVICE "Ready" "CP4BA Automation Decision Service Rest-API service status"

  CP4BA_ADS_REST_API_ZENINTEGRATION=$(jq -r .status.components.adsRestApi.adsRestApiZenIntegration ${CUR_DIR}/CR.json)
...
if [[ ${CP4BA_COMPONENTS/decisions_ads/} =~ "decisions" ]]; then
  ...
  if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "decisionCenter" ]]; then
    ...
    CP4BA_ODM_DECISION_CENTER_SERVICE=$(jq -r .status.components.odm.odmDecisionCenterService ${CUR_DIR}/CR.json)
    checkResult $CP4BA_ODM_DECISION_CENTER_SERVICE "Ready" "CP4BA Operational Decision Manager Decision Center service status"

    CP4BA_ODM_DECISION_CENTER_DEPLOYMENT=$(jq -r .status.components.odm.odmDecisionCenterDeployment ${CUR_DIR}/CR.json)
    ...
    CP4BA_ODM_DECISION_CENTER_ZENINTEGRATION=$(jq -r .status.components.odm.odmDecisionCenterZenIntegration ${CUR_DIR}/CR.json)
  ...
  if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "decisionRunner" ]]; then
    ...
    CP4BA_ODM_DECISION_RUNNER_SERVICE=$(jq -r .status.components.odm.odmDecisionRunnerService ${CUR_DIR}/CR.json)
    checkResult $CP4BA_ODM_DECISION_RUNNER_SERVICE "Ready" "CP4BA Operational Decision Manager Decision Runner service status"

    CP4BA_ODM_DECISION_RUNNER_DEPLOYMENT=$(jq -r .status.components.odm.odmDecisionRunnerDeployment ${CUR_DIR}/CR.json)
    ...
    CP4BA_ODM_DECISION_RUNNER_ZENINTEGRATION=$(jq -r .status.components.odm.odmDecisionRunnerZenIntegration ${CUR_DIR}/CR.json)
  ...
  if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "decisionServerRuntime" ]]; then
    ...
    CP4BA_ODM_DECISION_SERVER_RUNTIME_SERVICE=$(jq -r .status.components.odm.odmDecisionServerRuntimeService ${CUR_DIR}/CR.json)
    ...
    CP4BA_ODM_DECISION_SERVER_RUNTIME_DEPLOYMENT=$(jq -r .status.components.odm.odmDecisionServerRuntimeDeployment ${CUR_DIR}/CR.json)
    ...
    CP4BA_ODM_DECISION_SERVER_RUNTIME_ZENINTEGRATION=$(jq -r .status.components.odm.odmDecisionServerRuntimeZenIntegration ${CUR_DIR}/CR.json)
    ...
    logInfo "Checking Decision Server Runtime..."
    CP4BA_ODM_DECISION_SERVER_CONSOLE_SERVICE=$(jq -r .status.components.odm.odmDecisionServerConsoleService ${CUR_DIR}/CR.json)
    ...
    CP4BA_ODM_DECISION_SERVER_CONSOLE_DEPLOYMENT=$(jq -r .status.components.odm.odmDecisionServerConsoleDeployment ${CUR_DIR}/CR.json)
    ...
    CP4BA_ODM_DECISION_SERVER_CONSOLE_ZENINTEGRATION=$(jq -r .status.components.odm.odmDecisionServerConsoleZenIntegration ${CUR_DIR}/CR.json)
...
if [[ $CP4BA_COMPONENTS =~ "workflow" ]]; then
  if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "baw_authoring" ]]; then
    ...
    CP4BA_BAW_TYPE=$(jq -r .status.components.baw ${CUR_DIR}/CR.json | jq 'type')
    if [[ "$CP4BA_BAW_TYPE" == "object" ]]; then
      # only 1 baw instance
        CP4BA_BAW_DEPLOYMENT=$(jq -r .status.components.baw.bawDeployment ${CUR_DIR}/CR.json)
        checkResult $CP4BA_BAW_DEPLOYMENT "Ready" "CP4BA Business Automation Workflow Runtime deployment status"

        CP4BA_BAW_SERVICE=$(jq -r .status.components.baw.bawService ${CUR_DIR}/CR.json)
        checkResult $CP4BA_BAW_SERVICE "Ready" "CP4BA Business Automation Workflow Runtime service status"

        CP4BA_BAW_ZENINTEGRATION=$(jq -r .status.components.baw.bawZenIntegration ${CUR_DIR}/CR.json)
      ...
      for ((i=0; i<$CP4BA_BAW_INSTANCE_COUNT; i++)); do
        ...
        CP4BA_BAW_DEPLOYMENT=$(jq -r .status.components.baw[$i].bawDeployment ${CUR_DIR}/CR.json)
        checkResult $CP4BA_BAW_DEPLOYMENT "Ready" "CP4BA Business Automation Workflow Runtime deployment status"

        CP4BA_BAW_SERVICE=$(jq -r .status.components.baw[$i].bawService ${CUR_DIR}/CR.json)
        checkResult $CP4BA_BAW_SERVICE "Ready" "CP4BA Business Automation Workflow Runtime service status"

        CP4BA_BAW_ZENINTEGRATION=$(jq -r .status.components.baw[$i].bawZenIntegration ${CUR_DIR}/CR.json)
        checkResult $CP4BA_BAW_ZENINTEGRATION "Ready" "CP4BA Business Automation Workflow Runtime Zen Integration status"
      done
    fi
  fi
...
if [[ $CP4BA_COMPONENTS =~ "document_processing" ]]; then
  ...
  CP4BA_ADP_DEPLOYMENT=$(jq -r .status.components.ca.caDeployment ${CUR_DIR}/CR.json)
  checkResult $CP4BA_ADP_DEPLOYMENT "Successful" "CP4BA Automation Document Processing deployment status"

  CP4BA_ADP_SERVICE=$(jq -r .status.components.ca.caService ${CUR_DIR}/CR.json)
  checkResult $CP4BA_ADP_SERVICE "Ready" "CP4BA Automation Document Processing service status"

  CP4BA_ADP_ZEN_REGISTRATION=$(jq -r .status.components.ca.caZenRegistration ${CUR_DIR}/CR.json)
  ...
  if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "document_processing_designer" ]]; then
    ...
    CP4BA_ADP_CDS_DEPLOYMENT=$(jq -r .status.components.contentDesignerService.cdsDeployment ${CUR_DIR}/CR.json)
    checkResult $CP4BA_ADP_CDS_DEPLOYMENT "Ready" "CP4BA Automation Document Processing Designer deployment status"

    CP4BA_ADP_CDS_SERVICE=$(jq -r .status.components.contentDesignerService.cdsService ${CUR_DIR}/CR.json)
    checkResult $CP4BA_ADP_CDS_DEPLOYMENT "Ready" "CP4BA Automation Document Processing Designer service status"

    CP4BA_ADP_CDS_ZENINTEGRATION=$(jq -r .status.components.contentDesignerService.cdsZenIntegration ${CUR_DIR}/CR.json)
    ...
    CP4BA_ADP_CDRA_DEPLOYMENT=$(jq -r .status.components.contentDesignerRepoAPI.cdraDeployment ${CUR_DIR}/CR.json)
    ...
    CP4BA_ADP_CDRA_SERVICE=$(jq -r .status.components.contentDesignerRepoAPI.cdraService ${CUR_DIR}/CR.json)
    checkResult $CP4BA_ADP_CDRA_SERVICE "Ready" "CP4BA Automation Document Processing Content Designer Repo API service status"

    CP4BA_ADP_CDRA_ZENINTEGRATION=$(jq -r .status.components.contentDesignerRepoAPI.cdraZenIntegration ${CUR_DIR}/CR.json)
...
if [[ $CP4BA_OPTIONAL_COMPONENTS =~ "pfs" ]]; then
  if [[ $CP4BA_VERSION =~ "21.0.3" ]]; then
    ...
    CP4BA_PFS_DEPLOYMENT=$(jq -r .status.components.pfs.pfsDeployment ${CUR_DIR}/CR.json)
    checkResult $CP4BA_PFS_DEPLOYMENT "Ready" "CP4BA Process Federation Server deployment status"

    CP4BA_PFS_SERVICE=$(jq -r .status.components.pfs.pfsService ${CUR_DIR}/CR.json)
    checkResult $CP4BA_PFS_SERVICE "Ready" "CP4BA Process Federation Server service status"

    CP4BA_PFS_ZENINTEGRATION=$(jq -r .status.components.pfs.pfsZenIntegration ${CUR_DIR}/CR.json)
