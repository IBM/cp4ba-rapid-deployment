...
## Overwrite setup variables if using script to configure ADP database 

if [[ $* == *--adp* ]]
then
   echo 
   echo "Installing DB instance for ADP."
   ...
   db2Cpu=$adpDb2Cpu
   db2Memory=$adpDb2Memory
   db2NumOfDBsSupported=$adpDb2NumOfDBsSupported
   db2StorageSize=$adpDb2StorageSize
   db2OnOcpStorageClassName=$adpDb2OnOcpStorageClassName
   db2AdminUserName=$adpDb2AdminUserName
else
   echo 
   echo "Installing DB instance for CloudPak." 
fi
...
oc get configmap c-db2ucluster-db2dbmconfig -n $db2OnOcpProjectName -o yaml | sed "s|NUMDB 8|NUMDB ${db2NumOfDBsSupported}|" |  oc replace configmap -n $db2OnOcpProjectName --filename=-
