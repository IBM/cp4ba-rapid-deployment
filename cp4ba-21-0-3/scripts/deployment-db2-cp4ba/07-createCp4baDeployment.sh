...
if [[ $db2UseOnOcp == true ]]; then
  ...
  rm -f ./jdbc/db2/*
  oc cp -c db2u c-db2ucluster-db2u-0:/opt/ibm/db2/V11.5.0.0/java/db2jcc4.jar ./jdbc/db2/db2jcc4.jar
  oc cp -c db2u c-db2ucluster-db2u-0:/opt/ibm/db2/V11.5.0.0/java/db2jcc_license_cu.jar ./jdbc/db2/db2jcc_license_cu.jar
  oc project $cp4baProjectName
fi
...
then
  projectDBNumber=1
  while [[ $projectDBNumber -le $numberTenantDBs ]]
  do
      ## Format Project DB name of the form PDBXX where XX is a number between 01-99
      if [[ ${projectDBNumber} -le 9 ]]
      then
        projectDBName="${db2TenantDBPrefix}0${projectDBNumber}"
      else
        projectDBName="${db2TenantDBPrefix}${projectDBNumber}"
      fi
      ## Compile list of tenant names for later use
      if [[ -z ${adpProjectList} ]]
      then
        adpProjectList="${projectDBName}"
      else
        adpProjectList="${adpProjectList},${projectDBName}"
      fi
      ## Update aca-basedb secret with project db password so that it can be used by ADP
      ## the first two spaces are important to keep the yaml hierarchy
      echo "  ${projectDBName}_DB_CONFIG: \"${adpDb2AdminUserPassword}\"" >> adp-aca-basedb-secret.yaml

      projectDBNumber=$(($projectDBNumber + 1))
  done
fi
...
sed -i.bak "s|adpDb2HostName|$adpDb2HostName|g" ibm_cp4a_cr_final.yaml
sed -i.bak "s|adpDb2PortNumber|$adpDb2PortNumber|g" ibm_cp4a_cr_final.yaml
sed -i.bak "s|db2CaBasedbName|$db2CaBasedbName|g" ibm_cp4a_cr_final.yaml
sed -i.bak "s|adpTenantDatabases|$adpProjectList|g" ibm_cp4a_cr_final.yaml
...
if [[ $db2CreateAdpDbs == true ]]; then
  echo
  echo "Creating ADP aca secrets"
  oc apply -f adp-aca-basedb-secret.yaml
fi
