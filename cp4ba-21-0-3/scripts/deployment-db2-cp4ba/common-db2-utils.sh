...
function activateDatabase {
    ...
    DB=$(oc exec -c db2u c-db2ucluster-db2u-0 -it -- su - $dbUser -c "db2 list active databases " | grep ${db} | cat)
    ...
    oc exec c-db2ucluster-db2u-0 -it -- su - $dbUser -c "db2 activate database ${db}"
    rc=$?
    ...
    do
        ...
        sleep $waitTime
        oc exec c-db2ucluster-db2u-0 -it -- su - $dbUser -c "db2 deactivate database ${db}"
        sleep $waitTime
        oc exec c-db2ucluster-db2u-0 -it -- su - $dbUser -c "db2 activate database ${db}"
        rc=$?
        tries=$(( $tries + 1 ))
