...
@echo:
echo - This script will query the ACA Base database to determine if which project databases (if any) are marked for deletion...
@echo:
set /p base_db_name= "Enter the name of the ACA Base database. If nothing is entered, we will use the default value 'CABASEDB': "
IF NOT DEFINED base_db_name SET "base_db_name=CABASEDB"

@echo:
set /p base_db_user= "Enter the name of the database user for ACA Base database. If nothing is entered, we will use the default value 'CABASEUSER' : "
...
@echo:
echo Login to ACA Base database %base_db_name%
...
for /l %%j in (1,1,%DBCOUNT%) do (
  ...
  @echo: 
  echo Changing to 'imports' directory 
  cd imports

  @echo: 
  echo Running script:  ./importTables.sql
