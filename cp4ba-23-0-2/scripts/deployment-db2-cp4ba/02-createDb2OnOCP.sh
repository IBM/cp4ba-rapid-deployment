...
if [[ -f $DB2_INPUT_PROPS_FILENAME_FULL ]]; then
   ...
   if [ $db2OnOcpProjectName == "REQUIRED" ] || [ $db2AdminUserPassword == "REQUIRED" ] || [ "$db2StandardLicenseKey" == "REQUIRED" ]; then
      echo "File ${DB2_INPUT_PROPS_FILENAME} not fully updated. Pls. update all parameters BEFORE running this script."
      echo
      exit 1
   fi
...
echo ""
echo "Template $cp4baTemplateToUse will be used"
echo "The template requires $DBINSTANCES database instances. Each instance uses $db2Cpu cpus and $db2Memory memory."
echo ""
...
while [ $dbinst -le $DBINSTANCES ]; do
    ...
    cp db2.template.yaml db2-inst${dbinst}.yaml
    sed -i.bak "s|db2OnOcpInstanceName|db2-inst$dbinst|g" db2-inst${dbinst}.yaml
    sed -i.bak "s|db2OnOcpProjectName|$db2OnOcpProjectName|g" db2-inst${dbinst}.yaml
    sed -i.bak "s|db2OnOcpProjectName|$db2OnOcpProjectName|g" db2-inst${dbinst}.yaml
    sed -i.bak "s|db2AdminUserPassword|$db2AdminUserPassword|g" db2-inst${dbinst}.yaml
    sed -i.bak "s|db2InstanceVersion|$db2InstanceVersion|g" db2-inst${dbinst}.yaml
    sed -i.bak "s|db2Cpu|$db2Cpu|g" db2-inst${dbinst}.yaml
    sed -i.bak "s|db2Memory|$db2Memory|g" db2-inst${dbinst}.yaml
    sed -i.bak "s|db2StorageSize|$db2StorageSize|g" db2-inst${dbinst}.yaml
    sed -i.bak "s|db2OnOcpStorageClassName|$db2OnOcpStorageClassName|g" db2-inst${dbinst}.yaml
    ...
    sed -i.bak "s|db2License|$db2License|g" db2-inst${dbinst}.yaml
    oc apply -f db2-inst${dbinst}.yaml
    ...
    ##
    echo
    echo "Waiting up to 15 minutes for c-db2-inst${dbinst}-db2u statefulset to be created."
    date
    statefulsetQualifiedName=$(wait_for_resource_created_by_name statefulset c-db2-inst${dbinst}-db2u 15 $db2OnOcpProjectName)
    if [ -z "$statefulsetQualifiedName" ]
    then
	echo "Timed out waiting for db2-inst${dbinst}-db2u statefulset to be created by DB2 operator"
	exit 1
    fi
    
    echo
    echo "Patching db2-inst${dbinst}-db2u statefulset."
    if [ "$(oc get $statefulsetQualifiedName -n=$db2OnOcpProjectName -o yaml | grep 'tty')" != ""  ]
    then
    ...
    ##
    ## Wait for  db2-inst${dbinst}-restore-morph job to complte. If this job completes successfully
    ## we can tell that the deployment was completed successfully.
    ##
    echo
    echo "Waiting up to 30 minutes for db2-inst${dbinst}-restore-morph job to complete successfully."
    date
    jobStatus=$(wait_for_job_to_complete_by_name c-db2-inst${dbinst}-restore-morph 30 $db2OnOcpProjectName)
    if [ -z "$jobStatus" ]
    then
	echo "Timed out waiting for c-db2-inst${dbinst}-restore-morph job to complete successfully."
	exit 1
    fi
    ...
    ##
    echo
    echo "Updating number of databases allowed by DB2 installation from 8 to ${db2NumOfDBsSupported}."
    oc get configmap c-db2-inst${dbinst}-db2dbmconfig -n $db2OnOcpProjectName -o yaml | sed "s|NUMDB 8|NUMDB ${db2NumOfDBsSupported}|" |  oc replace configmap -n $db2OnOcpProjectName --filename=-
    
    echo
    echo "Updating database manager running configuration."
    oc exec c-db2-inst${dbinst}-db2u-0 -it -c db2u -- su - $db2AdminUserName -c "db2 update dbm cfg using numdb ${db2NumOfDBsSupported}"
    sleep $db2ActivationDelay #let DB2 settle down
    oc exec c-db2-inst${dbinst}-db2u-0 -it -c db2u -- su - $db2AdminUserName -c "db2set DB2_WORKLOAD=FILENET_CM"
    sleep $db2ActivationDelay #let DB2 settle down
    oc exec c-db2-inst${dbinst}-db2u-0 -it -c db2u -- su - $db2AdminUserName -c "set CUR_COMMIT=ON"
    sleep $db2ActivationDelay #let DB2 settle down

    echo
    echo "Restarting DB2 instance."
    oc exec c-db2-inst${dbinst}-db2u-0 -it -c db2u -- su -c "sudo wvcli system disable"
    sleep $db2ActivationDelay #let DB2 settle down
    oc exec c-db2-inst${dbinst}-db2u-0 -it -c db2u -- su - $db2AdminUserName -c "db2stop"
    sleep $db2ActivationDelay #let DB2 settle down
    oc exec c-db2-inst${dbinst}-db2u-0 -it -c db2u -- su - $db2AdminUserName -c "db2start"
    sleep $db2ActivationDelay #let DB2 settle down
    oc exec c-db2-inst${dbinst}-db2u-0 -it -c db2u -- su -c "sudo wvcli system enable"
    sleep $db2ActivationDelay #let DB2 settle down
...
while [ $dbinst -le $DBINSTANCES ]; do
    echo
    echo "Removing BLUDB from DB2 instance $dbinst"
    oc exec c-db2-inst${dbinst}-db2u-0 -it -c db2u -- su - $db2AdminUserName -c "db2 force application all"
    sleep $db2ActivationDelay #let DB2 settle down
    oc exec c-db2-inst${dbinst}-db2u-0 -it -c db2u -- su - $db2AdminUserName -c "db2 deactivate database BLUDB"
    sleep $db2ActivationDelay #let DB2 settle down
    oc exec c-db2-inst${dbinst}-db2u-0 -it -c db2u -- su - $db2AdminUserName -c "db2 drop database BLUDB"
    sleep $db2ActivationDelay #let DB2 settle down

    echo
    echo "Existing databases on DB instance #$dbinst are:"
    oc exec c-db2-inst${dbinst}-db2u-0 -it -c db2u -- su - $db2AdminUserName -c "db2 list database directory | grep \"Database name\" | cat"
...
dbinst=1
while [ $dbinst -le $DBINSTANCES ]; do
    workerNodeAddresses=$(get_worker_node_addresses_from_pod c-db2-inst${dbinst}-db2u-0 $db2OnOcpProjectName)
    echo -e "\tHostname instance ${dbinst}:${routerCanonicalHostname}"
    dbinst=$(( $dbinst + 1 ))
done
...
dbinst=1
while [ $dbinst -le $DBINSTANCES ]; do
    echo "Instance ${dbinst}: "
    oc get svc -n ${db2OnOcpProjectName} c-db2-inst${dbinst}-db2u-engn-svc -o json | grep nodePort
    dbinst=$(( $dbinst + 1 ))
done
