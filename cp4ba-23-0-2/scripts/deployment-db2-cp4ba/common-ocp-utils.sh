...
function wait_for_install_plan {
  ...
  while [ $CURRENT_WAIT_TIME -lt $TOTAL_WAIT_TIME_SECS ]
  do
    INSTALL_PLAN=$(oc get subscription.operators.coreos.com $subscriptionName -o custom-columns=IPLAN:.status.installplan.name --no-headers -n $namespace 2>/dev/null | grep -v "<none>" | cat)
    if [ ! -z "$INSTALL_PLAN" ] 
    then
      # Done waiting 
      break
