...
function prepare_olm_install() {
   ...
   for ((retry=0;retry<=${maxRetry};retry++)); do        
    ...
    if [[ $RUNTIME_MODE == "baw" || $RUNTIME_MODE == "baw-dev" ]]; then
        role_name_olm=$(${CLI_CMD} get role -n "$project_name" --no-headers|grep ibm-baw-operator.v|awk '{print $1}')
    else
...
function get_entitlement_registry(){
    ...
    while true; do
        ...
        if [[ -z "$CP4BA_AUTO_ENTITLEMENT_KEY" && ! -z "$CP4BA_AUTO_LOCAL_REGISTRY" ]]; then
            printf "\x1B[1mDo you have a Cloud Pak for Business Automation Entitlement Registry key (Yes/No, default: No):\x1B[0m No"
            ans="No"
        fi
        if [[ -z "$CP4BA_AUTO_LOCAL_REGISTRY" && ! -z "$CP4BA_AUTO_ENTITLEMENT_KEY" ]]; then
            printf "\x1B[1mDo you have a Cloud Pak for Business Automation Entitlement Registry key (Yes/No, default: No):\x1B[0m Yes"
            ans="Yes"
        fi
        ...
        "y"|"Y"|"yes"|"Yes"|"YES")
            ...
            if [[ "$RUNTIME_MODE" == "dev" || $RUNTIME_MODE == "baw-dev" ]]
            then
    ...
    while true; do
        if [ -z "$CP4BA_AUTO_PUSH_IMAGE_LOCAL_REGISTRY" ]; then
            ...
            "y"|"Y"|"yes"|"Yes"|"YES"|"True"|"TRUE"|"true")
                echo -e "\x1B[1mHave you pushed the images to the local registry using 'loadimages.sh' (CP4A images) (Yes/No)? \x1B[0m$CP4BA_AUTO_PUSH_IMAGE_LOCAL_REGISTRY"
                ans="Yes"
                ;;
...
function verify_silence_install(){
    if [[ ! -z "${CP4BA_AUTO_PLATFORM}" || ! -z "${CP4BA_AUTO_DEPLOYMENT_TYPE}" ]]; then
        ...
        local deployment_type_array=("demo" "enterprise")
        echo           "==========================================================================="
        echo -e "\x1B[1mStarting silent installation for Cloud Pak for Business Automation Operator\x1B[0m"
        echo           "==========================================================================="
        ...
        if [[ ! " ${deployment_type_array[@]} " =~ " ${CP4BA_AUTO_DEPLOYMENT_TYPE} " ]]; then
            echo -e "\x1B[1;31mOnly \"demo\" or \"enterprise\" is valid value for environment variable [CP4BA_AUTO_DEPLOYMENT_TYPE].\n\x1B[0m"
            exit 1
        fi
    fi

    if [[ "$CP4BA_AUTO_PLATFORM " == "other" && "$CP4BA_AUTO_DEPLOYMENT_TYPE" == "demo" ]]; then
        echo -e "\x1B[1;31mOnly \"enterprise\" is valid value for environment variable [CP4BA_AUTO_DEPLOYMENT_TYPE] when [CP4BA_AUTO_PLATFORM] set as \"other\" .\n\x1B[0m"
        exit 1        
    fi
