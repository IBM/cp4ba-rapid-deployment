...
if [[ $DBs =~ xUMSx ]]; then
    ...
    oc cp -c db2u createUMSDB.sh c-db2ucluster-db2u-0:/tmp/
    oc exec -c db2u c-db2ucluster-db2u-0 -it -- /bin/sh -c "chmod a+x /tmp/createUMSDB.sh"
    oc exec -c db2u c-db2ucluster-db2u-0 -it -- su - $db2AdminUserName -c "/tmp/createUMSDB.sh ${db2UmsdbName} ${db2AdminUserName}"
    oc exec -c db2u c-db2ucluster-db2u-0 -it -- /bin/sh -c "rm /tmp/createUMSDB.sh"
fi
